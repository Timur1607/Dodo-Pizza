export const data = {
    'news': [
        {
            'img': './pizzas-img/cezar/cezar-traditional-big.jpeg',
            'name': 'Цезарь',
            'price': 445,
        },
        {
            'img': './pizzas-img/tender-salmon/tender-salomon-traditional-large.jpeg',
            'name': 'Нежный лосось',
            'price': 495,
        },
        {
            'img': 'img/news/2pizza.png',
            'name': '2 пиццы',
            'price': 899,
        },
        {
            'img': './pizzas-img/carbonara/carbonara-traditional-medium.jpeg',
            'name': 'Карбонара',
            'price': 395,
        },
        {
            'img': './pizzas-img/carbonara/carbonara-traditional-medium.jpeg',
            'name': 'Карбонара',
            'price': 395,
        },
        {
            'img': './pizzas-img/carbonara/carbonara-traditional-medium.jpeg',
            'name': 'Карбонара',
            'price': 395,
        },
    ],

    'pizza': [

        {
            "name": "Пицца из половинок",
            "composition": "Соберите свою пиццу 35 см с двумя разными вкусами",
            "minimal-price": 600,
            "medium-price": 900,
            "big-price": 1025,
            "img": 'img/pizza/halfPizza.png',
            "assembly": true,
            "addition": [
                {
                    'img':'img/pizza/additive/cheese side.png',
                    'name':'Сырный бортик',
                    'nameENG': 'cheese side',
                    'price': 599,
                },
                {
                    'img':'img/pizza/additive/Champignons.jpg',
                    'name':'Шампиньоны',
                    'nameENG': 'Champignons',
                    'price':39 ,
                },
                {
                    'img':'img/pizza/additive/Chick.png',
                    'name':'Цыпленок',
                    'nameENG': 'Chick',
                    'price':59 ,
                },
                {
                    'img':'img/pizza/additive/feta cheese.png',
                    'name':'Брынза',
                    'nameENG': 'fetaCheese',
                    'price': 59,
                },
                {
                    'img':'img/pizza/additive/Spicy Jalapeno.png',
                    'name':'Острый халапенью',
                    'nameENG': 'spicyJalapeno',
                    'price':39 ,
                },
                {
                    'img':'img/pizza/additive/Spicy chicken.png',
                    'name':'Острый цыпленок',
                    'nameENG': 'spicyChicken',
                    'price': 59,
                },
            ],
            "ingredients": [
                'сыр чеддер'
            ],
        },
        {
            "name": "Пепперони-сердце",
            "composition": "Пикантная пепперони, моцарелла, томатный соус",
            "minimal-price": 625,
            "medium-price": 925,
            "big-price": 1045,
            "img": 'img/pizza/pizzaPepperoni.png',
            "ingredients": [
                'сыр чеддер'
            ],
            "addition": [
                {
                    'img':'img/pizza/additive/cheese side.png',
                    'name':'Сырный бортик',
                    'nameENG': 'cheese side',
                    'price': 599,
                },
                {
                    'img':'img/pizza/additive/Champignons.jpg',
                    'name':'Шампиньоны',
                    'nameENG': 'Champignons',
                    'price':39 ,
                },
                {
                    'img':'img/pizza/additive/Chick.png',
                    'name':'Цыпленок',
                    'nameENG': 'Chick',
                    'price':59 ,
                },
                {
                    'img':'img/pizza/additive/feta cheese.png',
                    'name':'Брынза',
                    'nameENG': 'fetaCheese',
                    'price': 59,
                },
                {
                    'img':'img/pizza/additive/Spicy Jalapeno.png',
                    'name':'Острый халапенью',
                    'nameENG': 'spicyJalapeno',
                    'price':39 ,
                },
                {
                    'img':'img/pizza/additive/Spicy chicken.png',
                    'name':'Острый цыпленок',
                    'nameENG': 'spicyChicken',
                    'price': 59,
                },
            ],
            
        },
        {
            "name": "Пирог-сердце",
            "composition": "Ананасы, брусника, сгущенное молоко",
            "minimal-price": 625,
            "medium-price": 925,
            "big-price": 1045,
            "img": 'img/pizza/pieHeart.png',
            "ingredients": [
                'сыр чеддер'
            ],
            "addition": [
                {
                    'img':'img/pizza/additive/cheese side.png',
                    'name':'Сырный бортик',
                    'nameENG': 'cheese side',
                    'price': 599,
                },
                {
                    'img':'img/pizza/additive/Champignons.jpg',
                    'name':'Шампиньоны',
                    'nameENG': 'Champignons',
                    'price':39 ,
                },
                {
                    'img':'img/pizza/additive/Chick.png',
                    'name':'Цыпленок',
                    'nameENG': 'Chick',
                    'price':59 ,
                },
                {
                    'img':'img/pizza/additive/feta cheese.png',
                    'name':'Брынза',
                    'nameENG': 'fetaCheese',
                    'price': 59,
                },
                {
                    'img':'img/pizza/additive/Spicy Jalapeno.png',
                    'name':'Острый халапенью',
                    'nameENG': 'spicyJalapeno',
                    'price':39 ,
                },
                {
                    'img':'img/pizza/additive/Spicy chicken.png',
                    'name':'Острый цыпленок',
                    'nameENG': 'spicyChicken',
                    'price': 59,
                },
            ],
        },
        {
            "name": "Чиззи чедер",
            "composition": "Ветчина, сыр чеддер, сладкий перец, моцарелла, томатный соус, чеснок, итальянские травы",
            "minimal-price": 625,
            "medium-price": 925,
            "big-price": 1045,
            "img": 'pizzas-img/cheesy-cheddar/cheesy-chadder-middle-traditional.jpeg',
            "ingredients": [
                'сыр чеддер'
            ],
            "addition": [
                {
                    'img':'img/pizza/additive/cheese side.png',
                    'name':'Сырный бортик',
                    'nameENG': 'cheese side',
                    'price': 599,
                },
                {
                    'img':'img/pizza/additive/Champignons.jpg',
                    'name':'Шампиньоны',
                    'nameENG': 'Champignons',
                    'price':39 ,
                },
                {
                    'img':'img/pizza/additive/Chick.png',
                    'name':'Цыпленок',
                    'nameENG': 'Chick',
                    'price':59 ,
                },
                {
                    'img':'img/pizza/additive/feta cheese.png',
                    'name':'Брынза',
                    'nameENG': 'fetaCheese',
                    'price': 59,
                },
                {
                    'img':'img/pizza/additive/Spicy Jalapeno.png',
                    'name':'Острый халапенью',
                    'nameENG': 'spicyJalapeno',
                    'price':39 ,
                },
                {
                    'img':'img/pizza/additive/Spicy chicken.png',
                    'name':'Острый цыпленок',
                    'nameENG': 'spicyChicken',
                    'price': 59,
                },
            ],
        },
        {
            "name": "Цыпленок блю чиз",
            "composition": "Цыпленок, сыр блю чиз, томаты, моцарелла, соус альфредо",
            "minimal-price": 445,
            "medium-price": 745,
            "big-price": 865,
            "img": './img/pizza/chickenBlueCheese.png',
            "ingredients": [
                'сыр чеддер'
            ],
            "addition": [
                {
                    'img':'img/pizza/additive/cheese side.png',
                    'name':'Сырный бортик',
                    'nameENG': 'cheese side',
                    'price': 599,
                },
                {
                    'img':'img/pizza/additive/Champignons.jpg',
                    'name':'Шампиньоны',
                    'nameENG': 'Champignons',
                    'price':39 ,
                },
                {
                    'img':'img/pizza/additive/Chick.png',
                    'name':'Цыпленок',
                    'nameENG': 'Chick',
                    'price':59 ,
                },
                {
                    'img':'img/pizza/additive/feta cheese.png',
                    'name':'Брынза',
                    'nameENG': 'fetaCheese',
                    'price': 59,
                },
                {
                    'img':'img/pizza/additive/Spicy Jalapeno.png',
                    'name':'Острый халапенью',
                    'nameENG': 'spicyJalapeno',
                    'price':39 ,
                },
                {
                    'img':'img/pizza/additive/Spicy chicken.png',
                    'name':'Острый цыпленок',
                    'nameENG': 'spicyChicken',
                    'price': 59,
                },
            ],
        },
        {
            "name": "Нежный лосось",
            "composition": "Лосось, томаты черри, соус песто, моцарелла, соус альфредо",
            "minimal-price": 495,
            "medium-price": 795,
            "big-price": 915,
            "img": './img/pizza/tenderSalomon.png',
            "ingredients": [
                'сыр чеддер'
            ],
            "addition": [
                {
                    'img':'img/pizza/additive/cheese side.png',
                    'name':'Сырный бортик',
                    'nameENG': 'cheese side',
                    'price': 599,
                },
                {
                    'img':'img/pizza/additive/Champignons.jpg',
                    'name':'Шампиньоны',
                    'nameENG': 'Champignons',
                    'price':39 ,
                },
                {
                    'img':'img/pizza/additive/Chick.png',
                    'name':'Цыпленок',
                    'nameENG': 'Chick',
                    'price':59 ,
                },
                {
                    'img':'img/pizza/additive/feta cheese.png',
                    'name':'Брынза',
                    'nameENG': 'fetaCheese',
                    'price': 59,
                },
                {
                    'img':'img/pizza/additive/Spicy Jalapeno.png',
                    'name':'Острый халапенью',
                    'nameENG': 'spicyJalapeno',
                    'price':39 ,
                },
                {
                    'img':'img/pizza/additive/Spicy chicken.png',
                    'name':'Острый цыпленок',
                    'nameENG': 'spicyChicken',
                    'price': 59,
                },
            ],
        },
        {
            "name": "Сырная",
            "composition": "Увеличенная порция моцареллы, сыры чеддер и пармезан, соус альфредо",
            "minimal-price": 445,
            "medium-price": 745,
            "big-price": 865,
            "img": './img/pizza/cheese.png',
            "ingredients": [
                'сыр чеддер'
            ],
            "addition": [
                {
                    'img':'img/pizza/additive/cheese side.png',
                    'name':'Сырный бортик',
                    'nameENG': 'cheese side',
                    'price': 599,
                },
                {
                    'img':'img/pizza/additive/Champignons.jpg',
                    'name':'Шампиньоны',
                    'nameENG': 'Champignons',
                    'price':39 ,
                },
                {
                    'img':'img/pizza/additive/Chick.png',
                    'name':'Цыпленок',
                    'nameENG': 'Chick',
                    'price':59 ,
                },
                {
                    'img':'img/pizza/additive/feta cheese.png',
                    'name':'Брынза',
                    'nameENG': 'fetaCheese',
                    'price': 59,
                },
                {
                    'img':'img/pizza/additive/Spicy Jalapeno.png',
                    'name':'Острый халапенью',
                    'nameENG': 'spicyJalapeno',
                    'price':39 ,
                },
                {
                    'img':'img/pizza/additive/Spicy chicken.png',
                    'name':'Острый цыпленок',
                    'nameENG': 'spicyChicken',
                    'price': 59,
                },
            ],
        },
        {
            "name": "Пепперони фреш",
            "composition": "Пикантная пепперони, увеличенная порция моцареллы, томаты, томатный соус",
            "minimal-price": 245,
            "medium-price": 545,
            "big-price": 665,
            "img": './img/pizza/pepperoniFresh.png',
            "ingredients": [
                'сыр чеддер'
            ],
            "addition": [
                {
                    'img':'img/pizza/additive/cheese side.png',
                    'name':'Сырный бортик',
                    'nameENG': 'cheese side',
                    'price': 599,
                },
                {
                    'img':'img/pizza/additive/Champignons.jpg',
                    'name':'Шампиньоны',
                    'nameENG': 'Champignons',
                    'price':39 ,
                },
                {
                    'img':'img/pizza/additive/Chick.png',
                    'name':'Цыпленок',
                    'nameENG': 'Chick',
                    'price':59 ,
                },
                {
                    'img':'img/pizza/additive/feta cheese.png',
                    'name':'Брынза',
                    'nameENG': 'fetaCheese',
                    'price': 59,
                },
                {
                    'img':'img/pizza/additive/Spicy Jalapeno.png',
                    'name':'Острый халапенью',
                    'nameENG': 'spicyJalapeno',
                    'price':39 ,
                },
                {
                    'img':'img/pizza/additive/Spicy chicken.png',
                    'name':'Острый цыпленок',
                    'nameENG': 'spicyChicken',
                    'price': 59,
                },
            ],
        },
    ],

    'combo': [
        {
            "name": "Комбо за 599 ₽",
            "description": "Наш хит «Аррива!» или другая пицца 25 см, Додстер, напиток и соус.",
            "price": 599,
            "img": "./img/combo/combo.png"
        },
        {
            "name": "2 пиццы",
            "description": "Самое популярное комбо из 2 пицц 30 см. Большой выбор.",
            "price": 899,
            "img": "./img/combo/combo.png"
        },
        {
            "name": "2 пиццы и закуска",
            "description": "2 пиццы 25 см и закуска на выбор. Для компании из 2–4 человек.",
            "price": 799,
            "img": "./img/combo/combo.png"
        },
        {
            "name": "Пицца и 2 закуски",
            "description": "Пицца 30 см и 2 закуски на выбор. Для компании из 2–4 человек.",
            "price": 799,
            "img": "./img/combo/combo.png"
        },
        {
            "name": "Комбо за 999 ₽",
            "description": "3 пиццы 30 см на тонком тесте по суперцене. Выбор пицц ограничен.",
            "price": 999,
            "img": "./img/combo/combo3pizza.png"
        }
    ],

    'snacks': [
        {
            "name": "Томатный суп с гренками",
            "description": "Горячий суп с пшеничными гренками на основе фирменного соуса из итальянских томатов",
            "price": 99,
            "img": "./img/snacks/Tomato soup with croutons.png",
        },
        {
            'name': "Томатный суп с митболами",
            'description': "Горячий суп с митболами из говядины и маслинами на основе фирменного соуса из итальянских томатов",
            'price': 149,
            'img': "./img/snacks/Tomato soup with meatballs.png",
        },
        {
            'name': "Додстер",
            'description': "Легендарная горячая закуска с цыпленком, томатами, моцареллой, соусом ранч в тонкой пшеничной лепешке",
            'price': 159,
            'img': "./img/snacks/Dodster.png",
        },
        {
            'name': "Острый Додстер",
            'description': "Горячая закуска с острым халапеньо, перцем, солеными огурчиками, томатами, моцареллой и соусом барбекю в тонкой пшеничной лепешке",
            'price': 159,
            'img': "./img/snacks/Chilly dodster.png",
        },
        {
            'name': "Додстер Чипотле",
            'description': "Горячая закуска с пикантным соусом чипотле из копченых перчиков, цыпленком, томатами, моцареллой в тонкой пшеничной лепешке",
            'price': 159,
            'img': "./img/snacks/Dodster Chipotle.png",
        }
    ],

    'desserts': [
        {
            'name': "Яблочный пирог",
            'description': "Десерт из песочного теста с яблочной начинкой. Может содержать изюм",
            'price': 99,
            'img': "./img/desserts/apple pie.png",
        },
        {
            'name': "Чизкейк Нью-Йорк",
            'description': "Классический американский творожный десерт",
            'price': 149,
            'img': "./img/desserts/Cheesecake New York.png",
        },
        {
            'name': "Фондан, 2 шт",
            'description': "Два десерта с хрустящей корочкой и теплой шоколадной начинкой",
            'price': 159,
            'img': "./img/desserts/Fondants, 2 pcs..png",
        },
        {
            'name': "Молочный коктейль с печеньем Oreo",
            'description': "Напиток из молока и мороженого с добавлением дробленого печенья «Oreo»",
            'price': 159,
            'img': "./img/desserts/Oreo milkshake.png",
        },
        {
            'name': "Классический молочный коктейль",
            'description': "Напиток из молока и мороженого",
            'price': 159,
            'img': "./img/desserts/Classic milkshake.png",
        },
    ],

    'drinks': [
        {
            'name': "2 Coca-Cola по суперцене",
            'description': "Две классические Coca-Cola по 0.5 литра по выгодной цене",
            'price': 99,
            'img': "./img/drinks/2 Coca-Cola at a super price.png",
        },
        {
            'name': "Coca-Cola Orange",
            'description': "0.5 л",
            'price': 149,
            'img': "./img/drinks/Coca-Cola Orange.png",
        },
        {
            'name': "Coca-Cola Zero",
            'description': "0.5 л",
            'price': 159,
            'img': "./img/drinks/Coca-Cola Zero.png",
        },
        {
            'name': "Coca-Cola",
            'description': "0.5 л",
            'price': 159,
            'img': "./img/drinks/Coca-Cola.png",
        },
        {
            'name': "Coca-Cola Vanilla",
            'description': "0.5 л",
            'price': 159,
            'img': "./img/drinks/Coca-Cola Vanilla.png",
        },
    ],

    'addition':[
        {
            'img': './img/addition/milkshake.png',
            'desc': 'Молочный коктейль с печеньем Oreo',
            'price': 175,
        },
        {
            'img': 'img/addition/rolls with lingonberries.png',
            'desc': 'Рулетики с брусникой',
            'price': 195,
        },
        {
            'img': 'img/addition/potatoes in the oven.png',
            'desc': 'Картофель из печи',
            'price': 175,
        },
    ],

    // 'pizzaAddition':[
    //     {
    //         'img':'img/pizza/additive/cheese side.png',
    //         'name':'Сырный бортик',
    //         'nameENG': 'cheese side',
    //         'price': 599,
    //     },
    //     {
    //         'img':'img/pizza/additive/Champignons.jpg',
    //         'name':'Шампиньоны',
    //         'nameENG': 'Champignons',
    //         'price':39 ,
    //     },
    //     {
    //         'img':'img/pizza/additive/Chick.png',
    //         'name':'Цыпленок',
    //         'nameENG': 'Chick',
    //         'price':59 ,
    //     },
    //     {
    //         'img':'img/pizza/additive/feta cheese.png',
    //         'name':'Брынза',
    //         'nameENG': 'fetaCheese',
    //         'price': 59,
    //     },
    //     {
    //         'img':'img/pizza/additive/Spicy Jalapeno.png',
    //         'name':'Острый халапенью',
    //         'nameENG': 'spicyJalapeno',
    //         'price':39 ,
    //     },
    //     {
    //         'img':'img/pizza/additive/Spicy chicken.png',
    //         'name':'Острый цыпленок',
    //         'nameENG': 'spicyChicken',
    //         'price': 59,
    //     },
    // ],
}